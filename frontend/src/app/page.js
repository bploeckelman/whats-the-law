    ...
    <main className={'bg-slate-800'}>
      ...
      <div className={''}>
        <div className="p-6 max-w-sm mx-auto bg-white rounded-xl shadow-lg flex items-center space-x-4">
          ...
          <div>
            <div className="text-xl font-medium text-black">You be talkin&apos;</div>
            <p className="text-slate-500">hello next world dot js dot tailwind css dot com</p>
          </div>
        </div>
      </div>
    </main>
    ...
    <div className='flex justify-center'>
      <div className='mb-3'>
        <div className='input-group relative flex flex-wrap items-stretch w-full mb-4'>
          ...
          <button type='button'
                  className='
                    btn flex items-center inline-block px-6 py-2.5
                    bg-blue-600
...
async function BillSearch1() {
  return (
    <div className='flex flex-row justify-center mb-20'>
      {/* selectbox for search criteria, congress, bill #, title, members, etc... */}
      <div className='relative w-1/2'>
        <input type='search'
               ...
               className='
                 ...
                 bg-gray-50
                 rounded-lg border-l-gray-50 border-l-2 border border-gray-300
                 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700
                 dark:border-l-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500'
        />
        <button type='submit'
                ...
                className='
                  ...
                  bg-blue-700
                  border border-blue-700 rounded-r-lg
                  hover:bg-blue-800
        ...
        <div id="dropdown-search-city" className="z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 block" style={{position: 'absolute', inset: '0px auto auto 0px', margin: '0px', transform: 'translate(0px, 72px)'}} data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="bottom">
            <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button-2">
                ...
                <li>
                    <button type="button" className="inline-flex py-2 px-4 w-full text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">
                        <div className="inline-flex items-center">
                            <svg aria-hidden="true" className="h-3.5 w-3.5 rounded-full mr-2" xmlns="http://www.w3.org/2000/svg" id="flag-icon-css-de" viewBox="0 0 512 512"><path fill="#ffce00" d="M0 341.3h512V512H0z"></path><path d="M0 0h512v170.7H0z"></path><path fill="#d00" d="M0 170.7h512v170.6H0z"></path></svg>
                            Germany
                        </div>
                    </button>
                </li>
...
async function Members() {
  ...
  return (
    <div className='w-1/2'>
      <h1 className='font-medium leading-tight text-5xl mt-0 mb-2 text-indigo-400'>Members</h1>
      <div className='p-10'>
        {members.map(member => <SimpleMember key={member.bioguideId} member={member} />)}
      </div>
    </div>
  );
}

async function SimpleMember({ member }) {
  return (
    <figure className='flex flex-row rounded-md shadow-lg m-4'>
      {/* might be easier to make member portrait a client component and catch 'onError' to set to default portrait instead of the background workaround */}
      <div className='w-1/3  bg-[url("/default-portrait.svg")] bg-[length:80px_80px] bg-center bg-no-repeat'>
        <div className='flex place-content-center'>
          {/*<Image*/} {/* NOTE - running the image through next results in a 403 */}
          <img
            className='rounded-full text-red-300 text-center align-bottom'
      ...
      <div className='w-2/3'>
        <blockquote className='mb-4'>
          <p className='text-xl font-medium text-sky-400'>{member.name}</p>
        </blockquote>

        <figcaption className='mb-4'>
          <p className='text-sm text-gray-400'>
            {member.party + ' from ' + member.state}
          </p>
          <p className='text-xs text-gray-500'>
            {
              Object.keys(member.served).map(body =>
                <p key={body}>
                {body}: { member.served[body].map(years => years.start + '-' + ((years.end === null) ? '' : years.end)) }
                </p>
              )
            }
          </p>
...
async function Bills() {
  ...
  return (
    <div className='w-full p-10'>
      <h1 className='font-medium leading-tight text-5xl mt-0 mb-2 text-indigo-400'>Bills</h1>
      <div className='w-3/4 p-10'>
        {bills.map(bill => <SimpleBill key={bill.title} bill={bill} />)}
      </div>
    </div>
  );
}
...
async function SimpleBill({ bill }) {
  return (
    <figure className='flex rounded-l-none rounded-r-full shadow-lg mb-8'>
      ...
      >
        <blockquote className='mb-4'>
          <p className='text-xl font-medium text-gray-200'>{bill.title}</p>
        </blockquote>
        ...
        <figcaption className=''>
          ...
          <p className='text-gray-400'>member information...</p>
        </figcaption>
      </div>
    </figure>
  );
...
async function Bill({ bill }) {
  return (
    <div className={'w-1/2 flex flex-row'}>
      ...
      {/* Content */}
      <div className="border-r border-r-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
        <div className="mb-2">
          <p className="text-sm text-gray-600 flex items-center">
            <svg className="fill-current text-500 w-3 h-3 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
              <path d="M4 8V6a6 6 0 1 1 12 0v2h1a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2v-8c0-1.1.9-2 2-2h1zm5 6.73V17h2v-2.27a2 2 0 1 0-2 0zM7 6v2h6V6a3 3 0 0 0-6 0z"/>
            </svg>
            {bill.latestAction.text}
          </p>
        </div>
        <div className="text-gray-900 font-bold text-xl mb-2">{bill.title}</div>
        <p className="text-gray-700 text-base">
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptatibus
          quia, Nonea! Maiores et perferendis eaque, exercitationem praesentium nihil.
        </p>
      </div>
      ...
      <div className="flex items-center">
        ...
        <div className="text-sm">
          ...
          <p className="text-gray-600">{bill.updateDate}</p>
        </div>
      </div>
